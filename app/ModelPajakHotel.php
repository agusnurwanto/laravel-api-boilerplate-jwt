...
class ModelPajakHotel extends Model
{
	public $table = 'pendataan_pajak_hotel';
	public $kc = 'm_kec';
	public $kl = 'm_kel';
	...
	public function getData($req, $count=false, $options=array()){
	    ...
	    if(!empty($count)){
			...
			$query = $query->select(DB::raw('p.*, kc.nama as kec, kl.nama as kel, bg.nama as jenis_hotel'));
		}
		$query = $query
	    	->leftjoin(
	    		$this->kc.' as kc', 
	    		function($q){
	    			$q->on('p.kd_kec_usaha', '=', 'kc.kd_kec')
	    				->on('p.kd_kab_usaha', '=', 'kc.kd_kab')
	    				->on('p.kd_prop_usaha', '=', 'kc.kd_prop');
	    	})
	    	->leftjoin(
	    		$this->kl.' as kl', 
	    		function($q){
	    			$q->on('p.kd_kel_usaha', '=', 'kl.kd_kel')
	    				->on('p.kd_kec_usaha', '=', 'kl.kd_kec')
	    				->on('p.kd_kab_usaha', '=', 'kl.kd_kab')
	    				->on('p.kd_prop_usaha', '=', 'kl.kd_prop');
	    	})
	    	->leftjoin(
	    		$this->bg.' as bg', 
	    		function($q){
	    ...
	    foreach ($req['columns'] as $k => $v) {
	    	...
	    	}else if($data=='jenis_hotel'){
	    		$s_jenis_hotel = $search;
	    	}
	    }
	    ...
	    if(!empty($s_jenis_hotel)){
	    	$query = $query->where('bg.nama', 'like', '%'.$s_jenis_hotel.'%');
	    }
