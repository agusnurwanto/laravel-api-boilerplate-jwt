...
class ModelPajakHiburan extends Model
{
	public $table = 'pendataan_pajak_hiburan';
	public $kc = 'm_kec';
	public $kl = 'm_kel';
	...
	public function getData($req, $count=false, $options=array()){
	    ...
	    if(!empty($count)){
			...
			$query = $query->select(DB::raw('p.*, kc.nama as kec, kl.nama as kel, bg.nama as jenis_hiburan'));
		}
		$query = $query
	    	->leftjoin(
	    		$this->kc.' as kc', 
	    ...
	    foreach ($req['columns'] as $k => $v) {
	    	...
	    	}else if($data=='jenis_hiburan'){
	    		$s_jenis_hiburan = $search;
	    	}
	    }
	    ...
	    if(!empty($s_persen_tarif)){
	    	$query = $query->where('p.persen_tarif', '=', $s_persen_tarif);
	    }
	    ...
	    if(!empty($s_jenis_hiburan)){
	    	$query = $query->where('bg.nama', 'like', '%'.$s_jenis_hiburan.'%');
	    }
        ...
        // echo "<pre>".print_r($req['columns'][$c_order]['data'], 1)."</pre>";
	    if(false){
  			$query = $query->toSql();
			dd($query);
		}
